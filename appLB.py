...
@cl.on_chat_start
async def start():
    actions = [cl.Action(name='upload', value='example_value', description='Click me!')]
...
async def main(message: cl.Message):
    query_engine: RetrieverQueryEngine | None = cl.user_session.get('query_engine')
    if query_engine is None:
        return  # OR raise here
...
async def on_action(_: cl.Action):
    ...
    if res is None:
        await cl.Message(content='No input received').send()
        return

    new_module: str | None = res.get('output')
    if new_module is None:
        await cl.Message(content='No module name provided').send()
        return

    await cl.Message(
        content=f'Your new module name is: {new_module}',
    ).send()
    ...
    await cl.Message(content=f'`{pdf_file.name}` uploaded successfully!').send()
